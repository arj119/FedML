...
from fedml_api.standalone.fedgan.server import FedGANAPI


class FedGANExperiment(ExperimentBase):
    algorithm_name = 'FedGAN'
    ...
    def experiment_start(self, client_model_config, client_models, args, device, dataset):
        assert len(
            client_models) == 1, f'FedGAN can only work with a single shared model, you have provided {len(client_models)}'
        generator = create_model(args, model_name=client_model_config['generator'], output_dim=dataset[7])
        discriminator, num_clients = client_models[0]
        api = FedGANAPI(dataset, device, args, generator, discriminator, num_clients)
        api.train()


if __name__ == "__main__":
    FedGANExperiment().start()
